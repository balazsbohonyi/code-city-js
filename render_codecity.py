...
def _district(path):
    """Full dotted folder path. The Java guide used the segment after `java/`
    or, failing that, only the immediate parent *name*, which merges every
    `components/` folder in a JS tree into one district. Folder path, slash
    to dot, is the JS package analog (ADR 0002 / 0004).
    """
...
if PKG_TSV.exists():
    with PKG_TSV.open(encoding="utf-8") as f:
...
if MOD_TSV.exists():
    ...
    with MOD_TSV.open(encoding="utf-8") as f:
        for row in csv.DictReader(f, delimiter="\t"):
...
.controls .spanAll { grid-column: 2 / -1; }
  ...
  /* Package-pattern filter: folder/name globs (..lib.* · use* · Planner*).
...
<section class="panel" id="settingsPanel">
  ...
  <h1>__LOGO_SVG__<span>__TITLE__</span><select id="viewMode" class="titleView"
      ...
      <option value="packages" id="packageOpt">Folders</option>
      <option value="modules" id="moduleOpt">Packages</option>
    </select></h1>
  <div class="controls">
    ...
    <label class="checkbox"
      ...
      <input id="colorLog" type="checkbox" checked aria-label="log colour ramp"> lg
    </label>
    ...
    <span class="knob">Folders</span>
    <select id="pkgLabelMode" aria-label="folder label style">
    ...
    <label class="checkbox spanAll" id="interPkgOnlyRow"
           title="Hold &#8997; over a building: draw only the roads that cross a folder boundary, and leave its couplings inside its own folder undrawn.">
      <input id="interPkgOnly" type="checkbox" aria-label="only roads that leave the folder">
      only where they leave the folder
    </label>
...
<div class="corner">
  ...
  <p class="cityorigin">Not an original idea: the software city is
    ...
    by Richard Wettel (USI Lugano, 2008). This is a JS/TS port of
    <a href="https://github.com/victorrentea/code-city" target="_blank" rel="noopener">Victor Rentea's Code City</a>.</p>
</div>
<section class="howto" id="howto" hidden>
  <div class="howto-card" role="dialog" aria-modal="true" aria-labelledby="howtoTitle">
    ...
    <h2 id="howtoTitle">Build a Code City for any JS/TS repo</h2>
    <p>This page is a self-contained snapshot of one repository. The same generators
      turn <em>any</em> git checkout of JavaScript or TypeScript &mdash; React, Vue,
      or no framework &mdash; into this 3-D city: a building per file, districts per
      folder. This project ports
      <a href="https://github.com/victorrentea/code-city" target="_blank" rel="noopener">Victor Rentea's Java Code City</a>
      to the JS/TS world. Point the pipeline at another repo and open the result:</p>
    <div class="howto-cmd">
...
const MODULES = __MODULES_JSON__;     // per-package.json-module rows (same shape) for module mode
...
// on a jumpable road or a building must not open that orbit, or Ctrl-click / Ctrl-
// double-click-to-source (esp. on Windows, where there is no separate ⌘) loses to a
// viewport rotate. Locked in capture-phase pointerdown before OrbitControls sees the
// event; cleared on pointerup / blur. Empty ground still orbits.
let sourceJumpLocksRotate = false;
// Remember the jump from pointerdown: click / dblclick fire after pointerup, and a
// 1–2px wiggle can clear unpinned streets (or miss the mesh) before the second pick.
let armedRoadJump = null;
let armedBuildingPath = null;

function releaseSourceJumpRotateLock() {
  if (!sourceJumpLocksRotate) return;
  controls.enableRotate = true;
  sourceJumpLocksRotate = false;
}
...
function onPointerDown(event) {
  ...
  isDragging = false;
  armedRoadJump = null;
  armedBuildingPath = null;
  if (event.metaKey || event.ctrlKey) {
    const jump = streetGroup ? pickRoadJump(event) : null;
    if (jump) {
      controls.enableRotate = false;
      sourceJumpLocksRotate = true;
      armedRoadJump = jump;
    } else {
      const hit = pickBuilding(event);
      if (hit && hit.object.userData.file) {
        controls.enableRotate = false;
        sourceJumpLocksRotate = true;
        armedBuildingPath = hit.object.userData.file.path;
      } else {
...
// Precedence: Cmd/Ctrl over a jumpable road or a building → hand (open-in-editor);
// otherwise Cmd/Ctrl → orbit glyph; an active drag → 4-way "move"; else hover cursor.
function applyCursor(event) {
  let cursor;
  // keydown/keyup carry modifiers but no clientX; reuse the last pointer position so
  // pressing Ctrl/⌘ while already over a building or jumpable road shows the hand,
  // not the orbit glyph.
  ...
  if (wantsMod && !isDragging && probe) {
    if (streetGroup && pickRoadJump(probe)) cursor = "pointer";
    else if (pickBuilding(probe)) cursor = "pointer";
    else cursor = ROTATE_CURSOR;
...
function hoverHeaderForFile(file) {
  ...
  if (view === "classes") {
    ...
    // Title shows the real filename (PlannerWorkspace.tsx), not the stripped stem.
...
function formatDistrictHover(district) {
  ...
  return identityHeader(shortName, "", pkg) +
    `<ul class="props"><li><span>files: <b>${count}</b></span></li></ul>`;
}
...
function onDoubleClick(event) {
  ...
  const hit = pickBuilding(event);
  const path = (hit && hit.object.userData.file && hit.object.userData.file.path)
            || armedBuildingPath;
  armedBuildingPath = null;
  if (!path) return;
  event.preventDefault();
...
// A suggestion picked from the datalist arrives with its " · 14 files" tail (the
...
const FAMILY_COUNT_TAIL = / *· *[0-9]+ +files *$/;
...
TOOL_REPO = "https://github.com/balazsbohonyi/code-city-js"
GUIDE_REPO = "https://github.com/victorrentea/code-city"
...
# 3. Open the city (Windows `start`; macOS `open`; Linux `xdg-open`).
start ~/workspace/your-repo/.codecity/codecity.html
"""

# Ready-made globs for the filter box's dropdown: folders first, then name
...
# Same as render_heatmap.py: the page is full of ⌘/⌥/⇧. Path.write_text()
# without encoding uses the Windows locale codec and raises UnicodeEncodeError.
