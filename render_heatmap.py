...
TITLE = os.environ.get("HEATMAP_TITLE", "Code City Codemap")
SUBTITLE = os.environ.get("HEATMAP_SUBTITLE", "")
...
for r in scatter_rows:
    if r["bug_commits"] >= 10 or (r["lines"] >= 200 and r["bugs_per_kloc"] >= 20):
        slabels.append(building_name(r["path"]))
    else:
        slabels.append("")
...
# HTML is UTF-8. Locale `open()` on Windows is cp1252, which cannot encode the
# ⌘ (U+2318) in the click-to-open hint — generate.py died there on vuejs/core
# before the city page was even reached. Never write this file as the locale.
